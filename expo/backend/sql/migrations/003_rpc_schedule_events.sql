-- =============================================================
-- MIGRATION 003: RPCs schedule events on action
-- =============================================================
-- Each action RPC (build, research, shipyard, fleet) now also
-- inserts a row into the events table so the event worker can
-- process completions independently of the world tick.
--
-- During the transition period, BOTH the world tick AND the
-- event worker will process completions. Both are idempotent:
-- - Event handler checks if timer/queue still exists before acting
-- - World tick deletes timer first (claim-by-delete)
-- =============================================================

-- =============================================================
-- 1. rpc_build_structure V2 (schedules building_complete event)
-- =============================================================
...
BEGIN
  ...
  v_end_time := v_now + v_duration_ms;

  INSERT INTO active_timers (user_id, planet_id, timer_type, target_id, target_level, start_time, end_time)
  VALUES (p_user_id, p_planet_id, 'building', p_building_id, v_target_level, v_now, v_end_time)
  RETURNING id INTO v_timer_id;

  UPDATE planets SET last_update = v_now WHERE id = p_planet_id;

  -- Schedule building_complete event
  v_execute_at := to_timestamp(v_end_time / 1000.0);
  v_idempotency_key := 'building:' || p_planet_id::text || ':' || p_building_id;

  INSERT INTO events (
    event_type, entity_type, entity_id, payload, execute_at,
    idempotency_key, priority, version, status
  ) VALUES (
    'building_complete', 'planet', p_planet_id,
    jsonb_build_object(
    ...
    v_execute_at, v_idempotency_key, 10, 2, 'pending'
  )
  ...
  RETURN json_build_object(
    ...
    'timer', json_build_object('id', p_building_id, 'type', 'building', 'targetLevel', v_target_level, 'startTime', v_now, 'endTime', v_end_time)
  );
END;
$$ LANGUAGE plpgsql;

-- =============================================================
-- 2. rpc_start_research V2 (schedules research_complete event)
-- =============================================================
...
BEGIN
  ...
  INSERT INTO active_timers (user_id, planet_id, timer_type, target_id, target_level, start_time, end_time)
  VALUES (p_user_id, NULL, 'research', p_research_id, v_target_level, v_now, v_end_time)
  RETURNING id INTO v_timer_id;

  UPDATE planets SET last_update = v_now WHERE id = p_planet_id;

  -- Schedule research_complete event
  v_execute_at := to_timestamp(v_end_time / 1000.0);
  v_idempotency_key := 'research:' || p_user_id::text || ':' || p_research_id;

  INSERT INTO events (
    event_type, entity_type, entity_id, payload, execute_at,
    idempotency_key, priority, version, status
  ) VALUES (
    'research_complete', 'player', p_user_id,
    jsonb_build_object(
      'player_id', p_user_id,
  ...
  RETURN json_build_object(
    'success', true,
    'resources', json_build_object('fer', v_fer, 'silice', v_silice, 'xenogas', v_xenogas, 'energy', v_energy),
    'timer', json_build_object('id', p_research_id, 'type', 'research', 'targetLevel', v_target_level, 'startTime', v_now, 'endTime', v_end_time)
  );
END;
$$ LANGUAGE plpgsql;

-- =============================================================
-- 3. rpc_build_ships V2 (schedules shipyard_unit_complete event)
-- =============================================================
...
BEGIN
  ...
  ELSE
    ...
    -- Schedule first unit completion event (only for new queue)
    v_execute_at := to_timestamp(v_end_time / 1000.0);
    v_idempotency_key := 'shipyard:' || p_planet_id::text || ':' || p_ship_id || ':ship:0';

    INSERT INTO events (
      event_type, entity_type, entity_id, payload, execute_at,
      idempotency_key, priority, version, status
    ) VALUES (
      'shipyard_unit_complete', 'planet', p_planet_id,
      jsonb_build_object(
        'planet_id', p_planet_id,
        'item_id', p_ship_id,
        'item_type', 'ship',
...
-- =============================================================
-- 4. rpc_build_defenses V2 (schedules shipyard_unit_complete event)
-- =============================================================
...
BEGIN
  ...
  ELSE
    ...
    -- Schedule first unit completion event (only for new queue)
    v_execute_at := to_timestamp(v_end_time / 1000.0);
    v_idempotency_key := 'shipyard:' || p_planet_id::text || ':' || p_defense_id || ':defense:0';
...
-- =============================================================
-- 5. rpc_rush_timer V2 (cancels corresponding event)
-- =============================================================
...
BEGIN
  ...
  -- Cancel the corresponding event
...
-- =============================================================
-- 6. rpc_cancel_timer V2 (cancels corresponding event)
-- =============================================================
...
-- =============================================================
-- 7. rpc_rush_shipyard V2 (cancels corresponding shipyard events)
-- =============================================================
...
-- =============================================================
-- 8. rpc_cancel_shipyard V2 (cancels corresponding shipyard events)
-- =============================================================
