...
export default function StatisticsScreen() {
  ...
  const { state, activePlanet, activeProduction, activeProductionPercentages, userId } = useGame();
  ...
  const ferPct = activeProductionPercentages?.ferMine ?? 100;
const silicePct = activeProductionPercentages?.siliceMine ?? 100;
const xenoPct = activeProductionPercentages?.xenogasRefinery ?? 100;
  ...
  const displayedMineProd = Math.floor(mineProd * (ferPct / 100));
  ...
  const mineLabel = ferPct === 100
    ? `Ferro Mine Nv.${ferLevel}`
    : `Ferro Mine Nv.${ferLevel} ${ferPct}%`;

  const lines: DetailLine[] = [
    { label: 'Production de base', value: `${formatNumber(baseProd)}/h` },
    { label: mineLabel, value: `${formatNumber(displayedMineProd)}/h` },
  ];

  if (plasmaLevel > 0) {
    lines.push({
      label: `Plasma Overdrive Nv.${plasmaLevel} (+${(plasmaBonus.fer * 100).toFixed(0)}%)`,
      value: `+${formatNumber(plasmaProd)}/h`,
      color: Colors.silice,
      indent: true,
    });
  }
  ...
  if (metalBonusPct > 0) {
    lines.push({
      label: `Bonus slot (+${metalBonusPct}%)`,
      value: `+${formatNumber(slotProd)}/h`,
  ...
  if (energyRatio < 1) {
    lines.push({
      label: `Ratio énergie ${Math.round(energyRatio * 100)}%`,
      value: `-${formatNumber(energyLoss)}/h`,
      color: Colors.danger,
      indent: true,
    });
  }
  ...
  return lines;
}, [ferLevel, ferPct, plasmaBonus.fer, plasmaLevel, metalBonusPct, energyRatio]);
  ...
  const displayedMineProd = Math.floor(mineProd * (silicePct / 100));
  ...
  const mineLabel = silicePct === 100
    ? `Silica Mine Nv.${siliceLevel}`
    : `Silica Mine Nv.${siliceLevel} ${silicePct}%`;
  ...
  if (plasmaLevel > 0) {
    lines.push({
      label: `Plasma Overdrive Nv.${plasmaLevel} (+${(plasmaBonus.silice * 100).toFixed(1)}%)`,
      value: `+${formatNumber(plasmaProd)}/h`,
      color: Colors.silice,
      indent: true,
    });
  ...
  if (crystalBonusPct > 0) {
    lines.push({
      label: `Bonus slot (+${crystalBonusPct}%)`,
      value: `+${formatNumber(slotProd)}/h`,
      color: Colors.primary,
      indent: true,
    });
  ...
  return lines;
}, [siliceLevel, silicePct, plasmaBonus.silice, plasmaLevel, crystalBonusPct, energyRatio]);
  ...
  const displayedMineProd = Math.floor(mineProd * (xenoPct / 100));
  ...
  const mineLabel = xenoPct === 100
    ? `Xeno Well Nv.${xenogasLevel}`
    : `Xeno Well Nv.${xenogasLevel} ${xenoPct}%`;
  ...
  if (plasmaLevel > 0) {
    lines.push({
      label: `Plasma Overdrive Nv.${plasmaLevel} (+${(plasmaBonus.xenogas * 100).toFixed(1)}%)`,
      value: `+${formatNumber(plasmaProd)}/h`,
      color: Colors.silice,
      indent: true,
    });
  ...
  lines.push({
    label: `Facteur temp. x${xenoTempFactor.toFixed(2)}`,
    value: `${tempEffect >= 0 ? '+' : ''}${formatNumber(tempEffect)}/h`,
    color: tempEffect >= 0 ? Colors.xenogas : Colors.danger,
    indent: true,
  });
  ...
  return lines;
}, [xenogasLevel, xenoPct, plasmaBonus.xenogas, plasmaLevel, xenoTempFactor, energyRatio]);
