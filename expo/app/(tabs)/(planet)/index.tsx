...
export default function PlanetScreen() {
  ...
  const quantumShieldYRef = useRef<number | null>(null);
  const [pendingScrollTarget, setPendingScrollTarget] = useState<string | null>(null);
  useEffect(() => {
    if (scrollTo) {
      setPendingScrollTarget(String(scrollTo));
    }
  }, [scrollTo, t]);

  useEffect(() => {
    if (pendingScrollTarget !== 'quantumShield') return;

    const timer = setTimeout(() => {
      const y = quantumShieldYRef.current;
  ...
  const PLANET_NAME_REGEX = /^[a-zA-Z0-9 _-]+$/;

const confirmRename = useCallback(() => {
  const trimmed = newPlanetName.trim();

  if (!trimmed) {
    showGameAlert('Erreur', 'Le nom ne peut pas être vide.');
    return;
  }

  if (trimmed.length > 12) {
    showGameAlert('Erreur', 'Le nom ne peut pas dépasser 12 caractères.');
    return;
  }

  if (!PLANET_NAME_REGEX.test(trimmed)) {
    showGameAlert('Erreur', 'Le nom ne peut contenir que des lettres, chiffres, espaces, tirets et underscores.');
    return;
  }
  ...
  const handleSaveUsername = useCallback(async () => {
    ...
    if (trimmed.length > 12) {
      showGameAlert('Erreur', 'Le pseudo ne peut pas dépasser 12 caractères.');
      return;
    }
  ...
  const onPlanetSwipeEnd = useCallback(
    (event: PanGestureHandlerGestureEvent) => {
      ...
      // On veut un vrai swipe horizontal, pas un scroll vertical
      if (Math.abs(translationX) < SWIPE_THRESHOLD) return;
      if (Math.abs(translationX) < Math.abs(translationY)) return;

      if (translationX < 0) {
        // swipe vers la gauche -> planète suivante
        goToNextPlanet();
      } else {
        // swipe vers la droite -> planète précédente
        goToPrevPlanet();
      }
    },
    [goToNextPlanet, goToPrevPlanet],
  );
  ...
  return (
    ...
    >
        ...
        <View style={styles.orbitalSection}>
          ...
          <View style={orbitalStyles.centerRow}>
            ...
            >
              <ChevronLeft
                size={24}
                color={planetIds.length > 1 ? Colors.textSecondary : Colors.border}
              />
            </TouchableOpacity>
      ...
      <View
        onLayout={e => {
          quantumShieldYRef.current = e.nativeEvent.layout.y;
        }}
      >
        <QuantumShieldCard />
      </View>
        ...
        >
          <Pressable style={styles.modalOverlay} onPress={() => setRenameModalVisible(false)}>
            <Pressable style={styles.modalContent} onPress={(e) => e.stopPropagation()}>
              ...
              />
              <Text style={styles.charCount}>{newPlanetName.length}/12</Text>
              <Pressable
