...
import { View, Text, StyleSheet, TouchableOpacity, RefreshControl, Alert, FlatList, ActivityIndicator } from 'react-native';
...
type ReportItem =
  | { type: 'espionage'; data: EspionageReport }
  | { type: 'combat'; data: CombatReport }
  | { type: 'transport'; data: TransportReport };
...
export default function ReportsScreen() {
  ...
  const {
    ...
    hasMoreEspionageReports,
    hasMoreCombatReports,
    hasMoreTransportReports,
    isFetchingMoreEspionageReports,
    isFetchingMoreCombatReports,
    isFetchingMoreTransportReports,
  } = useFleet();
  ...
  const data = useMemo<ReportItem[]>(() => {
    if (tab === 'espionage') {
      return espionageReports.map(r => ({ type: 'espionage', data: r }));
    }
    if (tab === 'combat') {
      return combatReports.map(r => ({ type: 'combat', data: r }));
    }
    return transportReports.map(r => ({ type: 'transport', data: r }));
  }, [tab, espionageReports, combatReports, transportReports]);

  const onEndReached = useCallback(() => {
    if (tab === 'espionage' && hasMoreEspionageReports && !isFetchingMoreEspionageReports) {
      void fetchMoreEspionageReports();
    } else if (tab === 'combat' && hasMoreCombatReports && !isFetchingMoreCombatReports) {
      void fetchMoreCombatReports();
    } else if (tab === 'transport' && hasMoreTransportReports && !isFetchingMoreTransportReports) {
      void fetchMoreTransportReports();
    }
  }, [
    tab,
    hasMoreEspionageReports,
  ...
  }, [router, confirmDeleteOne, user?.id]);
  ...
  const ListFooterComponent = useCallback(() => {
    const isLoadingMore =
      (tab === 'espionage' && isFetchingMoreEspionageReports) ||
      (tab === 'combat' && isFetchingMoreCombatReports) ||
      (tab === 'transport' && isFetchingMoreTransportReports);
    ...
    return (
      <View style={styles.footerLoader}>
        ...
        <Text style={styles.footerLoaderText}>Chargement de plus de rapports...</Text>
      </View>
    );
  }, [
    tab,
