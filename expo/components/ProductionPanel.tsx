...
export default function ProductionModal({ visible, onClose }: ProductionModalProps) {
  ...
  const producers = useMemo((): ProducerRow[] => {
    ...
    if (ferLevel > 0) {
      const baseProd = 10 + Math.floor(30 * ferLevel * Math.pow(1.1, ferLevel) * (1 + plasmaBonus.fer));
      rows.push({
        key: 'ferMine',
        label: 'Ferro Mine',
        icon: Pickaxe,
    ...
    if (siliceLevel > 0) {
      const baseProd = 5 + Math.floor(20 * siliceLevel * Math.pow(1.1, siliceLevel) * (1 + plasmaBonus.silice));
      rows.push({
        key: 'siliceMine',
        label: 'Silica Mine',
        icon: Gem,
