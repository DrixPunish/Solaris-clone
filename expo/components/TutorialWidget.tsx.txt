  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        ...
        isActive && styles.chapterHeaderActive,
    ...
    >
      ...
      <View style={styles.chapterHeaderRight}>
        ...
        {isCollapsed ? (
          <ChevronDown size={16} color={isActive ? Colors.primary : Colors.textSecondary} />
        ) : (
          <ChevronUp size={16} color={isActive ? Colors.primary : Colors.textSecondary} />
        )}
      </View>
    </Pressable>
  );
...
export function TutorialFullModal({
  ...
  const applyTutorialReward = useGame() as ReturnType<typeof useGame> & {
    applyTutorialReward?: (r: TutorialReward, stepId?: string) => Promise<{ success: boolean; error?: string }>;
  };

  const router = useRouter();
  const scrollRef = useRef<ScrollView>(null);
  const stepOffsetsRef = useRef<Record<string, number>>({});
  ...
  const buildInitialCollapsedState = useCallback(() => {
    const next: Record<string, boolean> = {};

    for (const chapter of allChapters) {
      const chapterSteps = allSteps.filter(s => s.chapterId === chapter.id);
      const isCurrentChapter = chapterSteps.some(step => step.id === currentStep?.id);
      const isChapterComplete =
        chapterSteps.length > 0 && chapterSteps.every(step => claimedRewards.includes(step.id));

      next[chapter.id] = isCurrentChapter ? false : isChapterComplete;
    }

    return next;
  }, [allChapters, allSteps, currentStep?.id, claimedRewards]);

  useEffect(() => {
    if (!visible) return;
    setCollapsedChapters(buildInitialCollapsedState());
  }, [visible, buildInitialCollapsedState]);
  ...
  const scrollToCurrentStep = useCallback(() => {
    if (!visible || !currentStep?.id) return;

    const y = stepOffsetsRef.current[currentStep.id];
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [visible, currentStep, buildInitialCollapsedState, scrollToCurrentStep]);
  ...
  const handleClaimFromList = useCallback(
    async (stepId: string) => {
      ...
      try {
        ...
        if (!applyTutorialReward?.applyTutorialReward) {
          setClaimingId(null);
          setIsClaimLoading(null);
          return;
        }

        const result = await applyTutorialReward.applyTutorialReward(reward, stepId);
        ...
        if (result.success) {
          ...
          if (
            result.error?.includes('not validated') ||
            result.error?.includes('complete the objective')
          ) {
            setClaimError('Objectif pas encore détecté. Ressayez dans quelques secondes.');
        ...
        void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
        setClaimError('Erreur réseau. Ressayez.');
      } finally {
        setClaimingId(null);
        setIsClaimLoading(null);
      }
    ...
    (navigateTo?: string) => {
      if (navigateTo) {
        ...
        setTimeout(() => {
          ...
          router.navigate(`${navigateTo}${sep}t=${Date.now()}` as never);
        }, 300);
      }
    },
    [router, onClose]
        ...
        <View style={styles.modalContainer}>
          ...
          >
            {allChapters.map(chapter => {
              ...
              const lastStepOrder = chapterSteps[chapterSteps.length - 1]?.order ?? 0;
              const isChapterActive =
                !isChapterComplete &&
                currentStepIndex >= firstStepOrder &&
                currentStepIndex <= lastStepOrder;
                    ...
                    chapterSteps.map(step => {
                      ...
                      return (
                        <Animated.View
                          key={step.id}
                          onLayout={event => {
                            stepOffsetsRef.current[step.id] = event.nativeEvent.layout.y;
                          }}
                          style={[
                            styles.stepCard,
                        ...
                        >
                          ...
                          <View style={styles.stepContent}>
                            ...
                            >
                              {isLocked
                                ? 'Complétez les missions précédentes...'
                                : step.description}
                            </Text>
                            ...
                            {canClaim && (
                              <Pressable
                                style={[
                                  styles.claimButtonList,
                                  isClaimLoading === step.id && styles.claimButtonListLoading,
                                ]}
